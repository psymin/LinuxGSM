...
cmd_safe_update=( "su;safe-update" "command_safe_update.sh" "Update the server if players=0." )
...
cmd_postdetails=( "pd;postdetails" "command_postdetails.sh" "Post details to a  paste service (removing passwords)." )
cmd_postconsole=( "pc;postconsole" "command_postconsole.sh" "Post console log a paste service (removing ipv4 IPs)." )
...
cmd_dev_check_players=( "cp;check-players" "command_dev_check_players.sh" "Query the number of players." )
...
if [ "${shortname}" == "wf" ]; then
        currentopt+=( "${cmd_postconsole[@]}" "${cmd_safe_update[@]}")
fi
